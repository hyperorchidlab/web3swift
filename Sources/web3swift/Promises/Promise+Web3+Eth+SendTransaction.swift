...
//        print(transaction)
        ...
        public func sendTransactionPromise(_ transaction: EthereumTransaction, priKey:Data, transactionOptions: TransactionOptions? = nil) -> Promise<TransactionSendingResult> {
                let queue = web3.requestDispatcher.queue
                do {
                    guard let from = transactionOptions?.from else {
                        ...
                        try Web3Signer.signTX(transaction: &assembledTransaction, account: from, priKey: priKey)
