...
public struct Web3Signer {
    public static func signTX(transaction:inout EthereumTransaction, keystore: AbstractKeystore, account: EthereumAddress, password: String, useExtraEntropy: Bool = false) throws {
        ...
        public static func signTX(transaction:inout EthereumTransaction, account: EthereumAddress, priKey: Data, useExtraEntropy: Bool = false) throws {
          
            if (transaction.chainID != nil) {
                try EIP155Signer.sign(transaction: &transaction, privateKey: priKey, useExtraEntropy: useExtraEntropy)
            } else {
                try FallbackSigner.sign(transaction: &transaction, privateKey: priKey, useExtraEntropy: useExtraEntropy)
            }
        }
