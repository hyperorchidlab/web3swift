...
public class WriteTransaction: ReadTransaction {
    ...
    public func assemble(transactionOptions: TransactionOptions? = nil) throws -> EthereumTransaction {
        ...
        public func send(priKey:Data, transactionOptions: TransactionOptions? = nil) throws -> TransactionSendingResult {
                let queue = self.web3.requestDispatcher.queue
                let promiseRet = self.assemblePromise(transactionOptions: transactionOptions).then(on: queue) {
                        
                        transaction throws -> Promise<TransactionSendingResult> in
                        ...
                        return self.web3.eth.sendTransactionPromise(transaction,
                                                            priKey: priKey,
                                                            transactionOptions: cleanedOptions)
                }
                return try promiseRet.wait()
        }
}
